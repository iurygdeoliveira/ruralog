...
| R-07 | D-05, D-06 | **Painel de privacidade**: a indústria escolhe o que o QR mostra (perfil B2B x B2C) | Checkboxes de privacidade que filtram a landing | Could |
...
| RP-02 | Usabilidade | Agricultor de baixa familiaridade digital não usaria um app complexo | Média | UI do Campo com pouquíssimos campos, linguagem simples, mobile-first |
| RP-03 | Técnico | Demo quebrar ao vivo (sem internet, estado sujo, QR não abre) | Alta | Rodar em localhost, botão "Resetar Demo", QR testado no celular antes |
...
|---|---|---|
| Tamanho e composição do time | 3 devs (Campo / Indústria+Varejo / Consumidor+Design), front-end vanilla | Sem backend, sem DevOps; tudo client-side estático |
...
|---|---|---|---|---|
| RF-01 | Cadastrar agricultor e propriedade (onboarding) | Nome, município (TO), cultura e geolocalização simulada salvos no lote | Must | D-06 |
...
| RF-03 | Disparar alerta preditivo simulado a partir do manejo | Registrar praga gera alerta (texto fixo por regra) visível à Indústria | Should | D-01 |
| RF-04 | Registrar colheita e gerar lote + QR Code | Lote `TO-2026-001` recebe ID, timestamp e QR apontando p/ landing | Must | D-06 |
...
| RF-09 | Exibir inbox de alertas preditivos na indústria | Alertas gerados no Campo aparecem como cards no painel | Should | D-01 |
| RF-10 | Receber lote no varejo e gerir validade | Dashboard mostra validade e semáforo; alerta de "vence em breve" | Should | D-04 |
| RF-11 | Painel de privacidade do QR (B2B/B2C) | Checkboxes definem o que a landing pública exibe | Could | D-05 |
| RF-12 | Landing pública do consumidor via QR (`?lote=`) | Abre por URL, exibe jornada/origem conforme privacidade | Must | D-05 |
...
|---|---|---|---|---|
| RNF-01 | Arquitetura | Apenas arquivos estáticos (HTML/CSS/JS), sem backend | Roda abrindo o arquivo / GitHub Pages | Time 3 devs, best-effort |
...
| C-02 | Sem build tools / sem npm em runtime | two-way | Arquivos abrem direto no navegador |
...
| D-03 | RF-07, RF-08 | RNF-02 | 3 |
| D-04 | RF-10 | RNF-02 | 4 |
| D-05 | RF-11, RF-12, RF-13 | RNF-04 | 4, 5 |
| D-06 | RF-01, RF-04, RF-14 | RNF-01, RNF-02 | 0, 1, 5 |
| RP-03 | RF-15 | RNF-03, RNF-07 | 0, 6 |
...
| 5 | RF-07, RF-08, RF-09 | Fase 3 | Lê lote + alertas |
...
**Could (só se sobrar tempo):** RF-11 completo, RF-13, múltiplos lotes, gráficos.
**Won't (fora do MVP):** auth real, IoT/GPS real, multiusuário, backend, app nativo.
