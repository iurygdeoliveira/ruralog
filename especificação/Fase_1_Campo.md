...
| **Entrega testável** | Percorrer onboarding → plantio → manejo → colheita e ver o QR gerado, com tudo persistido |
...
- `js/campo.js` (`type="module"`): importa `getLote/patchLote/addEvento/addAlerta` de `db.js`, `montarHeader/toast/botaoReset` de `ui.js`, e `qrcode` de `vendor/`.
...
1. **Onboarding (`RF-01`):** form com nome, município (select com cidades do TO), propriedade, cultura. Geo: botão "Usar localização" que grava uma coordenada **fixa simulada** do município (não depender de GPS real no palco). `patchLote("TO-2026-001", { agricultor, cultura })`.
...
3. **Manejo (`RF-03` + `RN-01`):** lista de ocorrências. Ao adicionar uma com tipo **"praga"**:
   ...
   await addAlerta(id, {
     ...
     severidade: "alta",
     mensagem: "IA RuraLog: praga detectada — previsão de redução de ~20% na entrega de " + lote.cultura,
     ts: Date.now()
   });
...
5. **QR Code (`RF-04`):**
   ...
   const qrUrl = base + "consumidor.html?lote=" + id;
...
- [ ] O QR renderiza e, escaneado no celular, abre `consumidor.html?lote=TO-2026-001`.
...
Login real, foto/upload, múltiplas culturas simultâneas, GPS real, validação de CPF. São Won't.
