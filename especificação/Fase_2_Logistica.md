...
| **Entrega testável** | Expedir o lote, registrar checkpoint manual, ver o cronômetro e o semáforo mudarem de cor por regra de tempo |
...
3. **Checkpoint manual (Should)** — botão que registra parada (`logistica.checkpoints`) para representar cargas sem GPS contínuo.

## 3. Componentes HTML/CSS/JS

- Uma `.rl-card` "Logística" em `index.html` com: botão **Expedir**, botão **Registrar checkpoint**, cronômetro, badge de semáforo (`ui.semaforo()`).
...
4. **Checkpoint manual (Should):** botão "Registrar parada/checkpoint" → push em `logistica.checkpoints` com `ts` e descrição curta. Também adicionar evento na timeline, por exemplo "Checkpoint logístico registrado". Não usar mapa nem GPS contínuo.
